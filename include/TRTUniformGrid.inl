...
namespace TinyRT
{
    ...
    void UniformGrid<ObjectSet_T>::Build( ObjectSet_T* pObjects, float fLambda )
    {
        ...
        m_cellCounts.x  = static_cast<uint32>( std::max( fTargetCells*vDiagonal.x, 1.0f ) );
        m_cellCounts.y = static_cast<uint32>( std::max( fTargetCells*vDiagonal.y, 1.0f ) );
        m_cellCounts.z = static_cast<uint32>( std::max( fTargetCells*vDiagonal.z, 1.0f ) );
