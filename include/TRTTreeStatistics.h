...
namespace TinyRT
{
    ...
    void GetTreeStatistics( TreeStatistics& rStats, const Tree_T* pTree )
    {
        ...
        while( !stack.empty() )
        {
            ...
            {
                rStats.nLeafs++;
                rStats.nMinLeafDepth = std::min( rStats.nMinLeafDepth, node.second );
                rStats.nMaxLeafDepth = std::max( rStats.nMaxLeafDepth, node.second );
                rStats.nAvgLeafDepth += node.second;
                
                size_t nObjects = pTree->GetNodeObjectCount( node.first );
                rStats.nMinObjectsPerLeaf = std::min( rStats.nMinObjectsPerLeaf, nObjects );
                rStats.nMaxObjectsPerLeaf = std::max( rStats.nMaxObjectsPerLeaf, nObjects );
