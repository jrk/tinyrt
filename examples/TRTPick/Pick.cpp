...
// File: Pick.cpp
//
// Direct3D mesh picking sample modified to use TinyRT
//
...
CDXUTMesh               g_Mesh;                 // The mesh to be rendered
...
TinyRT::AABBTree< TinyRT::StridedMesh<WORD> > g_BVH; // BVH used for TRT picking
TinyRT::ScratchMemory g_TRTScratch;                  // Scratch memory for TRT ray traversal
...
void InitTRTPicking( ID3DXMesh* pMesh );

void TRTPick( const D3DVERTEX* pVB, WORD* pIB, ID3DXMesh* pMesh, const D3DXVECTOR3* pOrigin, const D3DXVECTOR3* pDir );
...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    V_RETURN( g_Mesh.Create(pd3dDevice, L"assets\\scannerarm.x") );
    V_RETURN( g_Mesh.SetFVF(pd3dDevice, D3DVERTEX::FVF) );


    InitTRTPicking( g_Mesh.GetMesh() );
...
HRESULT Pick()
{
    ...
    if( GetCapture() )
    {
        ...
        else
        {
            // Not using D3DX
            g_dwNumIntersections = 0;
            TRTPick( pVertices, pIndices, pMesh, &vPickRayOrig, &vPickRayDir );
        }
...
void InitTRTPicking( ID3DXMesh* pMesh )
{
    // lock the mesh vertex and index buffers
    const D3DVERTEX* pVB;
    WORD* pIB;
    pMesh->LockVertexBuffer( D3DLOCK_READONLY, (void**) &pVB );
    pMesh->LockIndexBuffer( 0, (void**) &pIB );

    // construct a TRT strided mesh around the buffers
    WORD nVertices = (WORD) pMesh->GetNumVertices();
    DWORD nFaces = pMesh->GetNumFaces();
    DWORD nStride = pMesh->GetNumBytesPerVertex();
    TinyRT::StridedMesh<WORD> mesh( (const TinyRT::uint8*) pVB, pIB, nVertices, nFaces, nStride );
...
void TRTPick( const D3DVERTEX* pVB, WORD* pIB, ID3DXMesh* pMesh, const D3DXVECTOR3* pOrigin, const D3DXVECTOR3* pDir )
{
    // construct a TRT strided mesh around the locked buffers
    WORD nVertices = (WORD) pMesh->GetNumVertices();
    DWORD nFaces = pMesh->GetNumFaces();
    DWORD nStride = pMesh->GetNumBytesPerVertex();
    TinyRT::StridedMesh<WORD> mesh( (const TinyRT::uint8*) pVB, pIB, nVertices, nFaces, nStride );
    ...
    // cast a ray
    TinyRT::Ray ray( &pOrigin->x, &pDir->x );
    
    TinyRT::TriangleRayHit triHit;
    triHit.nTriIdx = 0xffffffff;
    TinyRT::RaycastBVH( &g_BVH, &mesh, ray, triHit, g_BVH.GetRoot(), g_TRTScratch );

    if( triHit.nTriIdx != 0xffffffff )
    {
        g_dwNumIntersections=1;
        g_IntersectionArray[0].dwFace = triHit.nTriIdx;
        g_IntersectionArray[0].fBary1 = triHit.vUVCoords[0];
        g_IntersectionArray[0].fBary2 = triHit.vUVCoords[1];
        g_IntersectionArray[0].fDist = ray.MaxDistance();
    }
